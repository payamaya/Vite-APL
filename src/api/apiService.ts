...
const apiService = {
  getAll: async <T>(endpoint: string): Promise<ApiResponse<T>> => {
    try {
      const response = await api.get<T>(endpoint) // Only use the endpoint, no need for baseURL
      return {
        data: response.data,
  ...
  getById: async <T>(endpoint: string, id: string): Promise<ApiResponse<T>> => {
    try {
      const response = await api.get<T>(`${endpoint}/${id}`) // Same as above, no need for baseURL
      return {
        data: response.data,
        status: response.status,
        statusText: response.statusText,
